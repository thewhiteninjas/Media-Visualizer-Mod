...
public final class AudioCaptureService {
    ...
    private void beginCapture(AudioDevice device) {
        AudioCaptureStrategy strategy = deviceManager.getStrategy();
        try {
    ...
    /**
     * Called when the platform-preferred strategy fails to start. Switches to the portable
     * {@link JavaSoundLoopbackStrategy} and retries once; if that also fails, gives up and
     * leaves {@link #running} false so the mod doesn't spin retrying forever.
     */
    ...
    private void restartWithConfiguredDevice() {
        AudioDevice device = deviceManager.selectDeviceById(config.outputDeviceId.get());
        deviceManager.getStrategy().stop();
        beginCapture(device);
    }
